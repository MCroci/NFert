...
- **F** = N da fertilizzazioni organiche anni precedenti (ammendanti 50/30/20 %; liquame bovino 30/15/10; suino/pollina 15/10/5).  
...
NFert provides three main categories of functions:

1. **Complete nitrogen balance calculation** - The `N_balance()` function
2. **Individual component functions** - For specific calculations (soil fertility, leaching, etc.)
3. **Precision agriculture functions** - For variable rate application based on NDVI
...
maize_params <- list(
  ...
  crop = "Mais trinciato classe 700",  # or "Maize silage (class 700)" (alias)
...
get_MAS("Frumento tenero (granella)", edition = "2025")  # 180 kg/ha
get_MAS("Mais da granella", edition = "2025")            # 280 kg/ha (irriguo)

# Check if a planned N dose is within MAS
check_MAS("Frumento tenero (granella)", 180)   # OK
check_MAS("Frumento tenero (granella)", 220)   # Over MAS
check_MAS("Mais da granella", 250)             # OK (MAS 260 or 280 depending on edition)
...
fields_data <- data.frame(
  ...
  crop = c("Soft wheat FF - strong (grain)", "Mais trinciato classe 700",
...
tryCatch({
  ...
  cat("Error:", e$message, "\n")
  cat("Solution: Use 'Grano tenero FF (granella)' instead\n")
})
```
