---
title: "Spatial Nitrogen Balance and Variable-Rate Prescription"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Spatial Nitrogen Balance and Variable-Rate Prescription}
...
2. **Compute** a spatially-explicit nitrogen balance with `spatial_N_balance()`
3. **Generate** a variable-rate (VRT) prescription with `variable_rate_N()` (Holland & Schepers method)

The workflow implements the pipeline described in Section 2.2 of the NFert
SoftwareX article: the field-scale balance provides the regulatory-compliant N
target, and the VRT functions redistribute it in space under a mass-balance
constraint.

## 1. Load the Cremonesi field rasters

NFert ships with six GeoTIFF rasters for a 12-ha arable field near Cremona
(Northern Italy), at ~3.5 m resolution: total nitrogen (TN, %), soil organic
matter (SOM, %), clay, sand and silt content (%), and the C/N ratio.
...
```{r plot-inputs, fig.width=7, fig.height=3}
...
`spatial_N_balance()` iterates `N_balance()` over every non-NA pixel in the
stack. Agronomic and climatic parameters (crop, yield, rainfall, previous
crop, organic history) are uniform; the spatial variability is driven by the
soil rasters alone.
...
```{r plot-balance, fig.width=5, fig.height=4}
...
In a real application the NDVI raster comes from UAV or satellite imagery.
Here we simulate one correlated with SOM (higher SOM → better canopy vigor):
...
```{r plot-vrt, fig.width=7, fig.height=3}
par(mfrow = c(1, 2), mar = c(3, 3, 2, 4))
...
`export_prescription()` polygonises a raster on the fly and writes any
of the seven formats accepted by modern on-board monitors (Shapefile,
GeoJSON, KML, GeoPackage, John Deere-ready, Trimble-ready, ISOXML
TASKDATA):

```{r export-multi, eval=FALSE}
...
When the field already has a defined A-B driving line, the strip
builder produces parallel polygons of the working width, each with a
dose derived from the VRT raster (or from NNI / VI directly):
