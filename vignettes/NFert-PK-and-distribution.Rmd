...
This vignette shows how to use **NFert 0.3.0** to compute a full nitrogen,
...
classify_SOM(SOM = SOM, soil_group = "Medio impasto")

# Maximum annual SO input (for ammendanti)
max_SO_input("Normale")
```
...
```{r mas}
get_MAS(crop = crop)
check_MAS(crop = crop, planned_N = n_dose)
```
...
When the cumulated dose overshoots `dose_max_N`, the function caps it and
...
- A list of organic applications (matrix name from `fert_org.table`,
  quantity in t/ha, year, modality-epoch id, optional efficiency level).
- A list of mineral applications (name from `concimi.table`, quantity in
  q/ha, modality-epoch).
...
plan <- plan_distribution(
  ...
  mineral_rows = list(
    list(concime       = "UREA AGRICOLA PRIL.46%",
         quantity_q_ha = 3,
         modality_epoch = 11)
  ),
  zvn = TRUE
...
Modalities are catalogued in `mod_distribuz.table` and their compatibility
with crop cycles in `ciclo_modalita.table`:

```{r mod}
head(NFert::mod_distribuz.table, 12)
```
...
head(NFert::fert_org.table[, c("fertilizer", "avg_ss", "avg_N",
                               "avg_P2O5", "avg_K2O", "zootec_100pct")], 10)

# Mineral: 146 commercial fertilisers
head(NFert::concimi.table, 10)
```
...
NFert::efficienza.table[
  NFert::efficienza.table$ID_Rag == 2 &
    NFert::efficienza.table$ID_Liv == 2 &
    NFert::efficienza.table$ID_Sett == "bov",
]
```
...
# NDVI raster: small Sentinel-2 example provided with NFert
data(s2.rast)
ndvi <- s2.rast
ndvi
```
...
# MAS (massimali) 2026
head(NFert::mas.table[, c("crop_id", "crop", "MAS_N", "N_standard",
                           "dose_max_N")], 5)
...
- The `plan_distribution()` function checks totals vs balance targets and
  ...
  (`ciclo_modalita.table`). A future release will add these checks.
